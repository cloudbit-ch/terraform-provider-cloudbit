...
func TestAccComputeCertificate_Basic(t *testing.T) {
	commonName := "cloudbit.ch"
	orgName := "Cloudbit GmbH"
		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeAggregateTestCheckFunc(
					resource.TestCheckResourceAttrSet("cloudbit_compute_certificate.foobar", "id"),
					resource.TestCheckResourceAttr("cloudbit_compute_certificate.foobar", "name", certificateName),
					resource.TestCheckResourceAttr("cloudbit_compute_certificate.foobar", "location_id", "1"),
					resource.TestCheckResourceAttr("cloudbit_compute_certificate.foobar", "certificate", certBase64),
					resource.TestCheckResourceAttr("cloudbit_compute_certificate.foobar", "private_key", privBase64),
					resource.TestCheckResourceAttrSet("cloudbit_compute_certificate.foobar", "info.not_before"),
					resource.TestCheckResourceAttrSet("cloudbit_compute_certificate.foobar", "info.not_after"),
					resource.TestCheckResourceAttrSet("cloudbit_compute_certificate.foobar", "info.serial_number"),
				),
			},
		},
	})
...
resource "cloudbit_compute_certificate" "foobar" {
	name        = "%s"
	location_id = 1
